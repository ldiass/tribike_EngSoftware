    ...
    signup_form.addEventListener("submit", (event) => {
      ...
      if(document.getElementById("sign_pwd").value!=document.getElementById("confirm_pwd").value){
		...
		function validateBankAccount(account) {
		 ...
		 // Validação do e-mail
		 if (!validateEmail(contaSubstring(document.getElementById("sign_mail").value)))
			 alert('E-mail inválido!');

		 // Validação do CPF
		 else if (!validateEmail(contaSubstring(document.getElementById("sign_cpf").value)))
			 alert('CPF inválido!');
		  
		 // Validação da data de nascimento
		 else if (!validateEmail(contaSubstring(document.getElementById("sign_dob").value)))
			 alert('Data de nascimento inválida!');
		  
		 // Validação do endereço
		 else if (!validateAddress(contaSubstring(document.getElementById("sign_end").value)))
			 alert('Endereço inválido!');
		  
		 // Validação da agência bancária
		 else if (!validateBankBranch(contaSubstring(document.getElementById("sign_ag").value)))
			 alert('Agência bancária inválida!');
		  
		 // Validação da conta corrente
		 else if (!validateBankAccount(contaSubstring(document.getElementById("sign_cc").value)))
